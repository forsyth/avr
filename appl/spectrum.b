...
init(ctxt: ref Draw->Context, args: list of string)
{
	...
	pidr := -1;
	cr := chan of (int, array of int);
	syncr := chan of int;
	spawn reader(sfd, cr, syncr);
	pidr = <-syncr;

	pids := -1;
	cs := chan of (int, array of int);
	syncs := chan of int;
	spawn spectrum(ctxt, cs, syncs);
	pids = <-syncs;

	pidh := -1;
	ch := chan of int;
	synch := chan of int;
	spawn history(ctxt, ch, synch);
	pidh = <-synch;
...
spectrum(ctxt: ref Draw->Context, cp: chan of (int, array of int), sync: chan of int)
{
	sync <-= sys->pctl(0, nil);
	(win, wmch) := tkclient->toplevel(ctxt, "", "Spectrum", Tkclient->Resize | Tkclient->Hide);
	...
	(img0, nil, nil) := newimage(win, ".p0");
	black := disp.rgb(0,0,0);
	white := disp.rgb(255,255,255);
	yellow := disp.color(Draw->Yellow);
	red := disp.color(Draw->Red);
	font := Font.open(disp, "/fonts/lucidasans/euro.8.font");

	scale := LOGSCALE;
	dx := real maxx/real NBINS;
	dy := real maxy/real (1<<scale);
	cmd(win, ".s set " + string scale);
	mindb := MAXDB;
	maxdb := MINDB;
	...
	for(;;){
		...
		c := <- cmdch =>
			(nil, w) := sys->tokenize(c, " ");
			case(hd w){
			"scale" =>
				# scale = int cmd(win, ".s get");
				scale = int hd tl w;
				if(scale < LOGMINSCALE)
					scale = LOGMINSCALE;
				if(scale > LOGMAXSCALE)
					scale = LOGMAXSCALE;
				dy = real maxy/real (1<<scale);
			"*" =>
				;
		...
		(db, p) := <-cp =>
			...
			if(db > maxdb)
				maxdb = db;
			hdraw(img, yellow, black, p, 0, dx, dy, maxy);
			img.text((maxx/4, maxy/16), white, (0, 0), font, string mindb);
			img.text((maxx/2, maxy/16), white, (0, 0), font, string db);
			img.text(((3*maxx)/4, maxy/16), white, (0, 0), font, string maxdb);
			img0.draw(img0.r, black, nil, Point(0, 0));
			y := T(db, maxy);
			img0.draw(Rect((0, maxy-y), (PWIDTH, maxy)), white, nil, (0, 0));
			y = T(mindb, maxy);
			img0.line((0, maxy-y), (PWIDTH, maxy-y), Endsquare, Endsquare, 0, red, (0, 0));
			y = T(maxdb, maxy);
			img0.line((0, maxy-y), (PWIDTH, maxy-y), Endsquare, Endsquare, 0, red, (0, 0));
			reveal(win, ".p");
			reveal(win, ".p0");
...
history(ctxt: ref Draw->Context, ci: chan of int, sync: chan of int)
{
	sync <-= sys->pctl(0, nil);
	(win, wmch) := tkclient->toplevel(ctxt, "", "Db", Tkclient->Resize | Tkclient->Hide);
	cmdch := chan of string;
	tk->namechan(win, cmdch, "cmd");
...
reader(fd: ref Sys->FD, cp: chan of (int, array of int), sync: chan of int)
{
	sync <-= sys->pctl(0, nil);
	p := array[NBINS] of int;
	syncs := 0;
	b := array[6] of { byte 16rfe, byte 16rdc, byte 16rba, byte 16r98, byte 16r76, byte 16r54 };
	buf := array[NSPEC] of byte;
loop:
	for(;;){
		if(syncs < 4){
			c := get1(fd, buf);
		...
		# db := get2(buf, k);
		db := (int buf[k])&16rff;
		if(c1 == b[syncs] && c2 == b[syncs+1]){	# otherwise buffer overflow
			cp <-= (db, p);
			if(0 && (db <= MINDB || db >= MAXDB)){
				sys->print("db=%d: ", db);
				for(i = 0; i < NBINS; i++)
					sys->print("%d ", p[i]);
				sys->print("\n");
...
winconfig := array[] of {
	"scale .s -length " + string MAXY + " -from " + string LOGMINSCALE + " -to " + string LOGMAXSCALE + " -orient vertical -showvalue 0 -command {send cmd scale}",
	"panel .p -width " + string MAXX + " -height " + string MAXY + " -bd 2 -relief flat",
	"panel .p0 -width " + string PWIDTH + " -height " + string MAXY + " -bd 2 -relief flat",
	"pack .s -side left -fill y",
	"pack .p -side left -fill both -expand 1",
	"pack .p0 -side right -fill x -fill y",
	"pack propagate . 0",
	"update"
};
